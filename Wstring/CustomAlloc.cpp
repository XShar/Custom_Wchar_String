...
/*
¬ данном решении выдел€етс€ статически 2ћб. пам€ти и работа идет с этим буфером.
¬ других реализаци€х можно вызвать heap_init(void* buf, size_t size) и передать нужный буфер и его размер.
*/
...
typedef struct segment {
	...
	struct segment* next;
	struct segment* prev;
} segment_t; // сегмент - как непрерывный кусок пам€ти
...
// находит сегмент свободной пам€ти, но никаких действий над ним не производит
// поиск производитс€ от переданного элемента и до конца списка
...
// возвращает указатель на новый сегмент пам€ти
...
// объедин€ет два соседних сегмента в один(первый поглощает второй)
...
void* _malloc(size_t size)
{
	...
	it->is_free = 0; // бронируем участок пам€ти

	// отрежем лишнюю пам€ть(на конце может остатьс€ маленький кусочек пам€ти)
...
void* _realloc(void* ptr, size_t size)
{
	...
	if (s->size == b) return ptr; // ничего делать не надо, размер не изменилс€
	else if (s->size > b) return ptr; // FIXME: надо бы освободить часть пам€ти
	else { // if (s->size < b)
