...
#define LENGHT_BITESET 32/sizeof(size_t)

static size_t byteset[LENGHT_BITESET];

static wchar_t* strchrnulW(const wchar_t* s, int c)
{
    size_t* w, k;
...
static size_t strspnW(const wchar_t* s, const wchar_t* c)
{
    const wchar_t* a = s;
...
static size_t strcspnW(const wchar_t* s, const wchar_t* c)
{
    const wchar_t* a = s;
    ...
    //Обнуление byteset
    unsigned int lenght_byteset = LENGHT_BITESET - 1;
    char* tmp_mem = (char*)byteset;
    while (lenght_byteset--) *tmp_mem++ = 0;
...
wchar_t* strCpyW(wchar_t* dest, const wchar_t* src)
{
    wchar_t* wcp = dest - 1;
    unsigned short c;
    const unsigned int off = src - dest + 1;
    do
    {
        c = wcp[off];
        *++wcp = c;
